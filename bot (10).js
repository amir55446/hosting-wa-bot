...
client.on('message_create', async (msg) => {
  try {
    ...
    // ─── خاص ───
    if (!chat.isGroup) {
      // ✅ تجاهل رسائل البوت في الخاص — نتحقق بـ flag + Set معاً
      if (selfChatLock) return;
...
// ✅ منع loop البوت في self-chat
// - selfChatBotMsgIds : IDs رسايل البوت (backup)
// - selfChatLock      : flag بيتضبط قبل الإرسال عشان نتجنب race condition
const selfChatBotMsgIds = new Set();
let   selfChatLock      = false;

async function selfReply(msg, text) {
  try {
    selfChatLock = true;                          // 🔒 قبل الإرسال
    const selfChat = await msg.getChat();
    const sent = await selfChat.sendMessage(text);
    if (sent?.id?._serialized) selfChatBotMsgIds.add(sent.id._serialized);
    setTimeout(() => { selfChatLock = false; }, 500); // فتح بعد 500ms
    return sent;
  } catch (e) {
    selfChatLock = false;
    return null;
...
async function selfSend(chat, text) {
  try {
    ...
    if (sent?.id?._serialized) selfChatBotMsgIds.add(sent.id._serialized);
    setTimeout(() => { selfChatLock = false; }, 500);
    return sent;
  } catch (e) {
    selfChatLock = false;
    return null;
